...
#else
#    PS1="\e[1;32m┌──[HQ🚀🌐\e[1;31m$(ip -4 addr sh dev eth0 | grep -v '127.0.0.1' | grep -v 'secondary' | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | sed -z 's/\n/|/g;s/|\$/\n/' | rev | cut -c 2- | rev) \e[1;32m[VPN]\e[1;31m$(ip -4 a sh dev tun0 2>/dev/null | grep -v '127.0.0.1' | grep -v 'secondary' | grep -oP '(?<=inet\s)\d+(\.\d+){3}')🔥\u\e[1;32m]\n└──╼[👾]\[\e[1;36m\]\$(pwd) $ \[\e[0m\]"
#fi
function prompt_func {
    LAST_STATUS=$?
    PS1="\e[1;32m┌──[HQ🚀🌐$(ip -4 addr sh dev eth0 | grep -v '127.0.0.1' | grep -v 'secondary' | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | sed -z 's/\n/|/g;s/|\$/\n/' | rev | cut -c 2- | rev)"
    if [[ $(ip a sh dev tun0 2>/dev/null) ]]; then
        PS1+="\e[1;31m⚔️Attacker:AppleTree📡IP:\e[1;31m$(ip -4 a sh dev tun0 2>/dev/null | grep -v '127.0.0.1' | grep -v 'secondary' | grep -oP '(?<=inet\s)\d+(\.\d+){3}')"
    fi
    PS1+="🔥\e[1;32m\u\e[1;32m]\n└──╼[👾]\[\e[1;36m\]\$(pwd) $ \[\e[0m\]"
}
